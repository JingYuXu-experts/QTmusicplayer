...
| Dify AI 助手 | 调用已发布的 Dify Workflow，提供音乐推荐、歌曲问答和播放器使用帮助 |
...
请求使用阻塞模式，已发布 Workflow 的实际输入变量为：

```text
user_query
```

Dify 返回的 `data.outputs` 可能包含多个输出字段，客户端会收集其中的非空文本、去除重复内容并显示在聊天气泡中。
...
确认发布后的 Workflow 输入变量仍为 `user_query`。如果在 Dify 中修改了开始节点变量名，需要同步更新 `inputKey`。
